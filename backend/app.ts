...
import { appConfig, readTurnstileEnvironment } from './config.js';
...
const TURNSTILE_COOKIE_NAME = 'phish_hunter_turnstile';
const TURNSTILE_SESSION_DURATION_MS = 12 * 60 * 60 * 1000;
...
export function createApp(dependencies: AppDependencies = {}) {
  ...
  const turnstileConfig = readTurnstileEnvironment();
  const turnstileSecret = turnstileConfig.secretKey;
  const sessionSecret = getSessionSecret(turnstileSecret);
  ...
  app.get('/api/public-config', (_request, response) => {
    response.status(200).json({
      ...
      turnstileEnabled: Boolean(turnstileConfig.secretKey),
    });
  });
  ...
  app.post('/api/turnstile/verify', async (request, response) => {
    ...
    try {
      const verification = await verifyTurnstileToken(token, request.ip, turnstileSecret);
      if (!verification.success) {
        response.status(401).json({
          error: 'turnstile_failed',
          message: 'Cloudflare Turnstile validation failed.',
        });
        return;
      }
      ...
      response.status(502).json({
        ...
        message: 'Cloudflare Turnstile validation is temporarily unavailable.',
      });
    }
  });
...
function getSessionSecret(turnstileSecret: string | null) {
  if (!turnstileSecret) {
    return null;
  }

  return process.env.APP_SESSION_SECRET?.trim() || turnstileSecret;
}
...
function createSessionToken(secret: string) {
  const payload = Buffer.from(JSON.stringify({ exp: Date.now() + TURNSTILE_SESSION_DURATION_MS })).toString('base64url');
  const signature = createHmac('sha256', secret).update(payload).digest('base64url');
  return `${payload}.${signature}`;
}

function hasValidSession(cookieHeader: string | undefined, secret: string) {
  const token = parseCookieHeader(cookieHeader)[TURNSTILE_COOKIE_NAME];
  if (!token) {
    return false;
  }
...
function buildSessionCookie(token: string) {
  const maxAgeSeconds = Math.floor(TURNSTILE_SESSION_DURATION_MS / 1000);
  return `${TURNSTILE_COOKIE_NAME}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Secure; Max-Age=${maxAgeSeconds}`;
}

function clearSessionCookie() {
  return `${TURNSTILE_COOKIE_NAME}=; Path=/; HttpOnly; SameSite=Lax; Secure; Max-Age=0`;
}
...
async function verifyTurnstileToken(token: string, remoteIp: string | undefined, secretKey: string) {
