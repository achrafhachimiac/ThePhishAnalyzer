...
describe('buildBrowserSandboxAccess', () => {
  ...
  it('builds a direct noVNC url from a full template with runtime placeholders', () => {
    const access = buildBrowserSandboxAccess(
      {
        ...
        accessMode: 'external',
        accessBaseUrl: null,
        accessUrlTemplate: 'http://203.0.113.10::novncPort/vnc.html?autoconnect=1&resize=remote',
        accessPathTemplate: ':jobId',
      },
      {
    ...
    expect(access.url).toBe('http://203.0.113.10:7612/vnc.html?autoconnect=1&resize=remote');
  });
