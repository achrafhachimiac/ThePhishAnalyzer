...
describe('backend app', () => {
  ...
  it('protects API and storage with a Turnstile-backed session cookie when configured', async () => {
    ...
    try {
      ...
      const publicConfig = await request(app).get('/api/public-config');
      expect(publicConfig.status).toBe(200);
      expect(publicConfig.body).toEqual({
      ...
      const turnstileResponse = await request(app)
        .post('/api/turnstile/verify')
        .send({ token: 'valid-turnstile-token' });

      expect(turnstileResponse.status).toBe(200);
      ...
      const sessionCookie = turnstileResponse.headers['set-cookie']?.[0];
      expect(sessionCookie).toContain('phish_hunter_turnstile=');
      ...
      if (previousTurnstileSiteKey === undefined) {
        ...
        process.env.TURNSTILE_SITE_KEY = previousTurnstileSiteKey;
      }
